...
def find_cycles():
    ...
    def recursive_helper(pkg, cycle):
        ...
        if len(cycle) >= max_len:  # if the cycle reaches the max length
            return
        if len(cycle) > 1 and cycle[0] == cycle[-1]:  # if a cycle is found
            if cycle not in cycles:  # delete duplicates
                cycles.append(cycle)
            return
        if len(cycle) > len(set(cycle)):  # a sub-cycle is found
            return
        for pkg_depend in pkg_dependency[pkg]:
            recursive_helper(pkg_depend, cycle + [pkg_depend])  # update cycle
